...
- An array of items which provide the `defaultValue` for the item as well as an auto-generated `key` to use when rendering the items.
- Helpers for manipulating the array.
- The array-level error message, if any.
