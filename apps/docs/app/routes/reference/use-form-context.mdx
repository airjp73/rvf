...
<PropHeader
  variant="h1"
  prop="useFormContext"
  type="(formId?: string) => FormContext"
/>
...
<div className="not-prose">
  <Alert
    ...
    details={
      <p>
        `useFormContext` is deprecated in favor of
        individual context selector hooks. Replacement hooks
        are noted in the documentation below for each
        individual property.
      </p>
    }
  />
...
Returns context about the form.

#### Usage

Normally, this hook is used inside of a `ValidatedForm`.
...
To use outside a `ValidatedForm`, pass a `formId` that matches the `id` prop of the form.

```tsx
const FormComponent = () => {
  const { isSubmitting } = useFormContext("myForm");
