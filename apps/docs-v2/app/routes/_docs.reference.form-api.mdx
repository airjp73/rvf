...
<Note>
  Even though these are functions, they're still tied to the render phase.
  Calling one of these functions in an event handler or a `useEffect` may not
  return the latest state. For those use-cases, you should use the
...
<Note>
  Fields added dynamically in a field array, will always be dirty after the
  first change.
</Note>
...
<button
  ...
  onClick={() => console.log(form.transient.value("myField"))}
>
  Log value
</button>
```
