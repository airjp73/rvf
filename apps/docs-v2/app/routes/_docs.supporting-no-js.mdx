...
<Note>
  This section applies to the plain React adapter. If you're using the `useForm`
  hook from the Remix adapter, read about [server-side validation for the remix
  adapter](/remix#server-side-validation).
</Note>

<Row>
  <Col>
    If you really need to support users who don't have JS enabled, you'll need to be rendering the form on your server.
