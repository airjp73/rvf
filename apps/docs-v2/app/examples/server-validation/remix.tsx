...
export const ServerValidationForm = () => {
  ...
  return (
    <form {...form.getFormProps()}>
      ...
      <MyInput scope={form.scope("firstName")} label="First name" />
      <MyInput scope={form.scope("lastName")} label="Last name" />
      <MyInput scope={form.scope("email")} label="Email" />

      {data && !isValidationErrorResponse(data) && <Note>{data.message}</Note>}
