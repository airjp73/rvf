...
// getting a weird error when calling `unstable_parseMultipartFormData`
it.todo(
  "should be able to submit multipart form data in stae mode",
  async () => {
    const validator = createValidator({
      validate: (data) => Promise.resolve({ data, error: undefined }),
    });
    ...
    const testUploadHandler = unstable_createFileUploadHandler();
    ...
    const Stub = createRemixStub([
      {
        ...
        Component: () => {
          ...
          const form = useRvf({
            // submitSource: "state",
            defaultValues: { foo: "", file: null as File | null },
            validator,
            method: "post",
          ...
          return (
            <form {...form.getFormProps()}>
              ...
              <input data-testid="foo" {...form.field("foo").getInputProps()} />
              <input data-testid="bar" name="file" type="file" />
              <button type="submit" data-testid="submit" />
            </form>
          );
        },
    ...
    await userEvent.type(screen.getByTestId("foo"), "bar");
    await userEvent.upload(screen.getByTestId("bar"), file);
    await userEvent.click(screen.getByTestId("submit"));

    expect(a).toHaveBeenCalledTimes(1);
    expect(await screen.findByText("You said: bar")).toBeInTheDocument();
    // Because we submitted as form data
    expect(a).toHaveBeenCalledWith({ foo: "bar", file: expect.any(File) });
  },
);
