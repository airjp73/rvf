...
describe("zfd v4 formData", () => {
  ...
  it("should handle arrays of objects", () => {
    const s = formData({
      todos: repeatable(
        z
          .object({ title: text(z.string()), description: text(z.string()) })
          .array(),
      ),
    });
  ...
  it("should combo well with other helpers", () => {
    const s = formData({
      name: text(z.string().optional()),
      checkboxGroup: repeatable(text(z.string()).array()),
    });
  ...
  it("should result fail if array is not present in FormData", () => {
    const s = formData({ myRepeatable: repeatable(z.any().array()) });
    expectError(s, new TestFormData());
  });

  it("should handle arrays of Files", () => {
    const s = formData({
      myRepeatable: repeatable(file().array()),
    });
